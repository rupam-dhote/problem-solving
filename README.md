<!-- Animated Header -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Poppins&weight=600&size=28&pause=1000&color=3B82F6&center=true&vCenter=true&width=700&lines=Data+Structures+%26+Algorithms;LeetCode+Problem+Solving;C%2B%2B+Solutions;Interview+Preparation" alt="Typing SVG" />
</p>

<h1 align="center">📘 DSA & LeetCode Problem Solving</h1>

<p align="center">
  A collection of my <b>Data Structures & Algorithms</b> solutions solved on <b>LeetCode</b> using <b>C++</b>.
  <br>
  This repository documents my coding journey and interview preparation.
</p>

<p align="center">
  <img src="https://img.shields.io/github/last-commit/rupam-dhote/problem-solving?style=for-the-badge">
  <img src="https://img.shields.io/github/repo-size/rupam-dhote/problem-solving?style=for-the-badge">
  <img src="https://img.shields.io/github/languages/top/rupam-dhote/problem-solving?style=for-the-badge">
  <img src="https://img.shields.io/github/license/rupam-dhote/problem-solving?style=for-the-badge">
</p>

---

# 🚀 Why This Repository?

This repository helps me:

- 🧠 Practice Data Structures & Algorithms daily
- 💡 Improve problem-solving skills
- 💼 Prepare for coding interviews
- 📈 Track my LeetCode journey
- 📚 Maintain clean and optimized C++ solutions

---

# 🧩 Topics Covered

- ✅ Arrays
- ✅ Strings
- ✅ Linked List
- ✅ Stack
- ✅ Queue
- ✅ Hashing
- ✅ Recursion
- ✅ Sorting
- ✅ Searching
- ✅ Binary Search
- ✅ Two Pointers
- ✅ Sliding Window
- ✅ Trees
- ✅ Graphs
- ✅ Greedy Algorithms
- ✅ Dynamic Programming
- ✅ Bit Manipulation

---

# 📖 Problem Source

Problems are solved from **LeetCode**.

Each solution generally includes:

- ✔️ Problem Link
- ✔️ Approach
- ✔️ Optimized Code
- ✔️ Time Complexity
- ✔️ Space Complexity

---

# 💻 Language

- C++

---

# 📂 Folder Structure

> **Automatically updated using GitHub Actions**

<!-- TREE_START -->

```text
.
├── .github
│   └── workflows
│       └── update-tree.yml
├── .gitignore
├── DSA
│   ├── Array
│   │   ├── 3sum.cpp
│   │   ├── BSTfromSortedArray.cpp
│   │   ├── binarySearch.cpp
│   │   ├── consecutiveOnes.cpp
│   │   ├── containDup.cpp
│   │   ├── findDisAppear.cpp
│   │   ├── findUnique.cpp
│   │   ├── firstAndLast.cpp
│   │   ├── highestAltitude.cpp
│   │   ├── intersectionOfArray2.cpp
│   │   ├── intesectionOfArray.cpp
│   │   ├── kIndices.cpp
│   │   ├── majority.cpp
│   │   ├── mergeSortedArray.cpp
│   │   ├── missingVal.cpp
│   │   ├── moveZeros.cpp
│   │   ├── pascalTriangle.cpp
│   │   ├── pascalsTriagleII.cpp
│   │   ├── plusOne.cpp
│   │   ├── profitloss.cpp
│   │   ├── rangeSumQuery.cpp
│   │   ├── ranges.cpp
│   │   ├── removeDupInsorted.cpp
│   │   ├── removeElement.cpp
│   │   ├── rotateK.cpp
│   │   ├── runingSum.cpp
│   │   ├── searchInRotatedSorted.cpp
│   │   ├── searchInsert.cpp
│   │   ├── sortArrayByparity.cpp
│   │   ├── sortByParityII.cpp
│   │   ├── twoSum.cpp
│   │   ├── valueDiff.cpp
│   │   └── volumeOfWater.cpp
│   ├── Math
│   │   ├── maxAchievableNumber.cpp
│   │   ├── numberToZero.cpp
│   │   ├── palindromNum.cpp
│   │   └── reverseInteger.cpp
│   └── String
│       ├── analog.cpp
│       ├── differenceInString.cpp
│       ├── excelColumn.cpp
│       ├── excelTitle.cpp
│       ├── firstOcuurence.cpp
│       ├── firstUnique.cpp
│       ├── fizzbuzz.cpp
│       ├── isPalindrome.cpp
│       ├── isValidBracket.cpp
│       ├── isomorphic.cpp
│       ├── largestNum.cpp
│       ├── lastWordLength.cpp
│       ├── logestCommonPrefix.cpp
│       ├── reverseString.cpp
│       ├── reverseovels.cpp
│       ├── stringRomanTOint.cpp
│       ├── subsequence.cpp
│       └── wordPattern.cpp
├── README.md
└── tree.txt

7 directories, 59 files
```

<!-- TREE_END -->

---

# ⭐ Repository Goals

- Solve LeetCode problems consistently
- Learn multiple approaches
- Write clean and optimized code
- Build strong DSA fundamentals
- Prepare for MAANG & Product-Based Companies

---

<p align="center">
⭐ If you find this repository helpful, don't forget to Star it!
</p>
